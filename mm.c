...
static void print_chunk_info(void* bp);
static void print_single_block(void* bp, int i);
static void* blk_bp_to_chnk_bp(void* bp);
static int debugger();
...
static int set_allocated(void *bp, size_t size, int blk_cntr) {
 ...
 //if(num_set_alloc == 1170) print_chunk_info(bp);
...
int mm_can_free(void *p)
{
  ...
  if(GET_SIZE(HDRP(p)) != GET_SIZE(FTRP(p))) {if(debug_on || 1){printf("CANNOT FREE HDR/FTR WITH UNEQUAL SIZES\n"); print_single_block(p, 0);} return 0;}
...
static void* blk_bp_to_chnk_bp(void* bp)
{
    ...
    void* original_bp;
    int error_count = 0;
    if(GET_SIZE(HDRP(bp)) == 0 && GET_SIZE(HDRP(bp) - sizeof(block_header)) != 0)
      bp = PREV_BLKP(bp);
